...
pack: out/UnMango.Docker.FSharp.$(VERSION).nupkg
...
PROJECT_FILE := src/FSharp/Docker.FSharp.fsproj
SRC := $(shell find src -maxdepth 2 -type f -name '*.fs')

out/UnMango.Docker.FSharp.$(VERSION).nupkg: $(SRC) $(PROJECT_FILE) README.md
